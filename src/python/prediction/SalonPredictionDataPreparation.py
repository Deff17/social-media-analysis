...
from python.clustering.ClusteringFeatures import start_date_clustering, end_date_clustering, end_date_salon, \
    start_date_salon
from python.database.Database import Database
from python.database.DbUtils import connection_SALON
...
users AS (SELECT user_id FROM  user_mapping WHERE date = '{date_start}'),
users_and_dates AS (SELECT date, user_id FROM dates, users),
user_labels AS (SELECT user_id, label, date FROM user_mapping WHERE date between '{date_start}' AND '{date_end}' AND user_id IN (SELECT user_id FROM user_mapping WHERE date = '{date_start}')),
...
def getVectorsWithNeighbours(db, date_start, date_end, length):
    ...
    user_neighbours AS (
        SELECT user_id,
               ...
               number_of_links_5,
               number_of_links_6
    FROM counted_neighbours_new WHERE date between '{date_start}' AND '{date_end}' AND user_id IN (SELECT user_id FROM user_mapping WHERE date = '{date_start}')),
    filled_lables AS (
        SELECT
    user_id,
                             label AS label,
...
def getPredictionData(db, start_date, end_date, vector_length, file_name):
    ...
    save_data_to_file("SALON_PREDICTION_SET_NOT_OVERSAMPLED", file_name, df)
...
def getPredictionDataNeighbours(db, start_date, end_date, vector_length, file_name):
    ...
    df = pd.DataFrame(result, columns=columns)
    save_data_to_file("SALON_PREDICTION_SET_NOT_OVERSAMPLED_NEIGHBOURS", file_name, df)


if __name__ == '__main__':
    db = Database(connection=connection_SALON)
    getPredictionData(db, start_date_salon, end_date_salon, 2, "only_roles_2.csv")
    getPredictionData(db, start_date_salon, end_date_salon, 3, "only_roles_3.csv")
    getPredictionData(db, start_date_salon, end_date_salon, 4, "only_roles_4.csv")
    getPredictionData(db, start_date_salon, end_date_salon, 5, "only_roles_5.csv")
    #print(get_quenry_for_length(2))
    getPredictionDataNeighbours(db, start_date_salon, end_date_salon, 2, "roles_with_neighbours_2.csv")
    getPredictionDataNeighbours(db, start_date_salon, end_date_salon, 3, "roles_with_neighbours_3.csv")
    getPredictionDataNeighbours(db, start_date_salon, end_date_salon, 4, "roles_with_neighbours_4.csv")
    getPredictionDataNeighbours(db, start_date_salon, end_date_salon, 5, "roles_with_neighbours_5.csv")
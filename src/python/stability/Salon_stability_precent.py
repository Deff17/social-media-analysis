...
def number_of_unchanged_roles_slots_plots(db, user_mapping, folder, cluster):
    ...
    SELECT CASE WHEN all_user_num != 0 THEN (the_same_role_for_2_slots::decimal / all_user_num::decimal) * 100.0 ELSE the_same_role_for_2_slots END,
           CASE WHEN all_user_num != 0 THEN (the_same_role_for_3_slots::decimal / all_user_num::decimal) * 100.0 ELSE the_same_role_for_3_slots END,
           CASE WHEN all_user_num != 0 THEN (the_same_role_for_4_slots::decimal / all_user_num::decimal) * 100.0 ELSE the_same_role_for_4_slots END,
           CASE WHEN all_user_num != 0 THEN (the_same_role_for_5_slots::decimal / all_user_num::decimal) * 100.0 ELSE the_same_role_for_5_slots END,
           CASE WHEN all_user_num != 0 THEN (the_same_role_for_6_slots::decimal / all_user_num::decimal) * 100.0 ELSE the_same_role_for_6_slots END,
           CASE WHEN all_user_num != 0 THEN (the_same_role_for_7_slots::decimal / all_user_num::decimal) * 100.0 ELSE the_same_role_for_7_slots END
    FROM dates d
    ...
    data.extend(res)
    df = pd.DataFrame(data,
                      columns=["the_same_role_for_2_slots_percentage",
                               "the_same_role_for_3_slots_percentage",
                               "the_same_role_for_4_slots_percentage",
                               "the_same_role_for_5_slots_percentage",
                               "the_same_role_for_6_slots_percentage",
                               "the_same_role_for_7_slots_percentage"])
    save_data_to_file(folder, f"stability_for_cluser_PERCENTAGE_{cluster}", df)

def plot_stability_for_cluser(folder, cluster, dest_folder):
    df = pd.read_csv(f"{folder}/stability_for_cluser_PERCENTAGE_{cluster}").fillna(0)
    df.plot(title="Stability for role {}".format(cluster_dict_salon.get(cluster)), figsize=(15, 8), legend=True)
    plt.ylabel("number of users")
    plt.xlabel("time slot")
    save_fig(dest_folder, f"stability_for_cluser_PERCENTAGE_{cluster}.png", plt)
    plt.show()
...
column_cluster_dict = {
    "0": "number_of_slots_as_expressive_commentator",
    "1": "number_of_slots_as_low_active_user",
    "2": "number_of_slots_as_active_user",
    "3": "number_of_slots_as_casual_commentator",
    "4": "number_of_slots_as_systematic_user",
    "5": "number_of_slots_as_influential_user",
    "6": "number_of_slots_as_engaging_blogger"
}
...
def SALON_stability():
    ...
    # number_of_changes_from_role_to_different(db, "user_mapping", "SALON_stability")
    # number_of_unchanged_roles_slots_ratio(db, "user_mapping", "SALON_stability")
    # print("DONE SALON_stability_ratio")

    # number_of_unchanged_roles_slots(db, "user_mapping", "SALON_stability")
    # print("DONE SALON_stability_general_stats")
    number_of_unchanged_roles_slots_plots(db, "user_mapping", "SALON_stability", "0")
    print("done 0")
    number_of_unchanged_roles_slots_plots(db, "user_mapping", "SALON_stability", "1")
    print("done 1")
    number_of_unchanged_roles_slots_plots(db, "user_mapping", "SALON_stability", "2")
    print("done 2")
    number_of_unchanged_roles_slots_plots(db, "user_mapping", "SALON_stability", "3")
    print("done 3")
    number_of_unchanged_roles_slots_plots(db, "user_mapping", "SALON_stability", "4")
    print("done 4")
    number_of_unchanged_roles_slots_plots(db, "user_mapping", "SALON_stability", "5")
    print("done 5")
    number_of_unchanged_roles_slots_plots(db, "user_mapping", "SALON_stability", "6")
    print("DONE SALON_stability")
    plot_stability_for_cluser(f"{base}/SALON_stability", "0", "SALON_STABILITY_PLOT")
    plot_stability_for_cluser(f"{base}/SALON_stability", "1", "SALON_STABILITY_PLOT")
    plot_stability_for_cluser(f"{base}/SALON_stability", "2", "SALON_STABILITY_PLOT")
    plot_stability_for_cluser(f"{base}/SALON_stability", "3", "SALON_STABILITY_PLOT")
    plot_stability_for_cluser(f"{base}/SALON_stability", "4", "SALON_STABILITY_PLOT")
    plot_stability_for_cluser(f"{base}/SALON_stability", "5", "SALON_STABILITY_PLOT")
    plot_stability_for_cluser(f"{base}/SALON_stability", "6", "SALON_STABILITY_PLOT")
    print("DONE plot")
    # get_general_roles_statistics(db, user_mapping="user_mapping", folder="SALON_stability")
    # print("DONE general stats")
    # get_cluster_different_roles_statistics(db, cluster='0', user_mapping="user_mapping", folder="SALON_stability")
    # get_cluster_different_roles_statistics(db, cluster='1', user_mapping="user_mapping", folder="SALON_stability")
    # get_cluster_different_roles_statistics(db, cluster='2', user_mapping="user_mapping", folder="SALON_stability")
    # get_cluster_different_roles_statistics(db, cluster='3', user_mapping="user_mapping", folder="SALON_stability")
    # get_cluster_different_roles_statistics(db, cluster='4', user_mapping="user_mapping", folder="SALON_stability")
    # get_cluster_different_roles_statistics(db, cluster='5', user_mapping="user_mapping", folder="SALON_stability")
    # get_cluster_different_roles_statistics(db, cluster='6', user_mapping="user_mapping", folder="SALON_stability")

...
from src.python.utils.util import save_fig, save_data_to_file, cluster_dict_salon, cluster_dict_salon_num, \
...
def plot_neighbours(db, counted_neighbours, user_mapping, folder, roles_dict):
    for clust in range(0, 7):
        ...
        for t in neighbour_types:
            plt.plot('date', str(t), data=cluser_neighbour_table, marker='', markerfacecolor=types_colors.get(t), label=f"Role: {cluster_dict_salon.get(t)}")
        # plt.yscale('symlog')
...
def nodes(db, start_date = '2008-01-01', end_date = '2013-07-06'):
    conn = db.conn
    cur = conn.cursor()
    query = f"""
    DROP TABLE Node;
...
def edges(db, start_date = '2008-01-01', end_date = '2013-07-06'):
    conn = db.conn
    cur = conn.cursor()
    query = f"""
    DROP TABLE Edge;
...
def outdegree(db):
    ...
    query = f"""
        ...
        do $$
        declare
            declare start_date date := '2008-01-01';
            declare end_date date := '2013-07-06';
        begin
        while start_date < end_date loop
...
def get_data(db):
    ...
    save_data_to_file_with_index("Salon_global_stats", "global_stats", df)


def analyse_neighbour_data_Salon():
    db = Database(connection_SALON)
    try:
        plot_neighbours(db, "counted_neighbours", "user_mapping", "PLOT_Salon_Neighbourhood_analysis", cluster_dict_salon_num)
        print("plotting done...")
        neighbours_picks_diffs_table(db, "counted_neighbours", "user_mapping", "Salon_Neighbourhood_analysis")
        print("neighbours picks done...")

        create_global_stats_neighbour(db)
        get_global_cluster_neighbour_data_new(db, "counted_neighbours_new", "user_mapping", "number_of_links_0")
        get_global_cluster_neighbour_data_new(db, "counted_neighbours_new", "user_mapping", "number_of_links_1")
        get_global_cluster_neighbour_data_new(db, "counted_neighbours_new", "user_mapping", "number_of_links_2")
        get_global_cluster_neighbour_data_new(db, "counted_neighbours_new", "user_mapping", "number_of_links_3")
        get_global_cluster_neighbour_data_new(db, "counted_neighbours_new", "user_mapping", "number_of_links_4")
        get_global_cluster_neighbour_data_new(db, "counted_neighbours_new", "user_mapping", "number_of_links_5")
        get_global_cluster_neighbour_data_new(db, "counted_neighbours_new", "user_mapping", "number_of_links_6")
        print("DONE feeding global stats")
        get_data(db)


        global_statistics(db, "counted_neighbours", "user_mapping", "Salon_Neighbourhood_analysis", "global_stats")
        print("neighbours general done...")
        perform_page_rank(db)
        print("page rank done...")
        page_rank_analysis(db, "PageRank", "user_mapping", "Salon_PageRank_analysis")
